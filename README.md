# Claude Code 学习文档

Claude Code 使用指南与实践记录，按主题分类。

## 目录

### 安装与配置

- [安装与初始配置](setup/installation.md)

### 日常使用

- [常用命令与快捷键速查表](usage/commands-shortcuts.md)

### 远程控制

- [Channels 配置（Telegram / Discord）](channels/channels-setup.md)

### 插件

- [Claude Code 官方插件指南](plugins/claude-code-plugins-guide.md)

### 工具

- [gstack 安装指南](tools/gstack-install.md)
- [Obsidian 集成指南](tools/obsidian-setup.md)
- [MCP 安装记录（Context7 / GitHub）](tools/mcp-install.md)

### 快速入门

- [OKG 内部 Claude 快速入门完整指南](quickstart/okg-claude-quickstart.md)

### 工作流

- [Vibe Coding 指南](workflow/vibe-coding-guide.md)
- [LLM Wiki 知识库方法论](workflow/llm-wiki-methodology.md)

### 内部原理

- [Claude Code 源码深度架构分析](internals/claude-code-architecture.md)
